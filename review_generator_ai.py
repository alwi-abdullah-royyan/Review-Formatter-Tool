...
def check_ai_usage():
    ...
    print("this will not tries to enhance h-scene, pros and cons part.")
...
def parse_sections(filename):
    ...
    return {
        key: '\n'.join(value).strip() if key in ['main','gameplay','combat','art','story','h-content' , 'tldr', 'game'] 
...
def generate_review_bbcode(sections):
    ...
    h_content = sections.get("h-content", "")
    ...
    body5_bbcode = '\n'.join([f'{line}' for line in h_content.split('\n') if line.strip()])
    if body5_bbcode:  
        body5_bbcode += '\n\n'
    ...
    return title_bbcode + body_bbcode + body1_bbcode + body2_bbcode + body3_bbcode + body4_bbcode + body5_bbcode + pros_bbcode + cons_bbcode + tldr_bbcode
